...
library(quanteda)
TextsEC <- corpus(MEX$text)
docvars(TextsEC, "Program") <- MEX$Program
...
SkillsEC <- data.frame(Country = "MEXador",
...
IMEC <- as_incidence_matrix(BNEC, names = TRUE, sparse = TRUE, types = bipartite_mapping(BNEC)$type)
IMEC2 <- as.matrix(IMEC)

rownames(ProgramsEC)[order(ProgramsEC$Eigenvector, decreasing = TRUE)]
selected_columns <- head(rownames(ProgramsEC)[order(ProgramsEC$Eigenvector, decreasing = TRUE)], 10)
# Let's pick the most important soft skills
# as per their eigenvector centrality

current_column_names <- colnames(IMEC2)

# Subset the matrix by column names
IMEC3 <- IMEC2[, selected_columns, drop = FALSE]

current_column_names <- colnames(IMEC3)
...
for (i in seq_along(current_column_names)) {
  ...
  skill_index <- match(current_column_names[i], SkillsEC$SkillCode)
  
  # If a match is found, map the skill code to its name
  if (!is.na(skill_index)) {
    mapped_skill_names[i] <- SkillsEC$Skill[skill_index]
  } else {
...
colnames(IMEC3) <- mapped_skill_names
colnames(IMEC3)

library(bipartite)
plotweb(IMEC3, method = "normal", 
        col.high = "#FCD116", 
        bor.col.high = "#FCD116",
        col.low = "#CE1127", 
        bor.col.low = "#CE1127",
